...
abstract class LiveData<T> {
  ...
  void observe(LiveDataObserver<T> observer, {bool emitCurrentValue = true}) {
    ...
    if (emitCurrentValue) observer(_currentValue);
  }
  ...
  void _updateValue(T value) {
    ...
    if (_currentValue != oldValue) _notifyObservers();
  }
...
/// }
/// ```
final class MutableLiveData<T> extends LiveData<T> {
...
class _LiveDataBuilderState<T> extends State<LiveDataBuilder<T>> {
  ...
  void initState() {
    ...
    _observer = (T value) => setState(() => _currentValue = value);
...
class LiveDataListener<T> extends StatefulWidget {
  ...
  @override
  State<StatefulWidget> createState() => _LiveDataListenerState<T>();
}
